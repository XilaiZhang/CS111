...
const char *usage = "[-v] [--script=file] [--timeout=seconds] program [args ...]";
...
int process_command(const char *cmd) {
	...
	static char expbuf[BUFSIZE];
 	...
 	if (strncmp("SEND", cmd, 4) == 0) {
		...
		write(outPipe[1], sendbuf, strlen(sendbuf));
		return(1);
...
int main( int argc, char **argv ) {
	...
	if (verbose) {
		...
		const char *t = timeout ? "--timeout" : "";
		fprintf(stderr, "%s %s %s%s %s\n",
			argv[0], v, s, t, script ? script : "");
	}
