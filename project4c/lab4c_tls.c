...
int main(int argc, char **argv){
  ...
  //sample code from above website tells us how to implement ssl
  OpenSSL_add_all_algorithms();
  SSL_load_error_strings();

  if(SSL_library_init()<0){fprintf(stderr,"openssl library could not be initialized");exit(1);}
  method=SSLv23_client_method();
  //try to create a new SSL context
  if((ctx=SSL_CTX_new(method))==NULL)
    {fprintf(stderr,"new SSL context structure cannot be created");exit(1);}
  ssl=SSL_new(ctx);
  ...
  if(SSL_connect(ssl)!=1){fprintf(stderr,"SSL session not built");exit(1);}
  
  //https://linux.die.net/man/3/ssl_write
  //sslwrite
  char sslbuffer[40];
  memset(sslbuffer,0,40);
  sprintf(sslbuffer,"ID=%s\n",id);
  SSL_write(ssl,sslbuffer,strlen(sslbuffer));
  ...
  while(1){
	...
	memset(big,0,500);
	memset(small,0,20);
	int num_read=SSL_read(ssl,big,500);
	//fprintf(stderr,"%s\n",big);
